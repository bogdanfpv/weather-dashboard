...
def handle_weather_request(apigw_client, connection_id, city, country, token, client_id):
    ...
    try:
        ...
        is_valid, message, token_timestamp = validate_and_consume_token(location_key, token)
...
def broadcast_token_status(apigw_client, location_key, is_available):
    ...
    try:
        ...
        try:
            token_response = rate_limit_table.get_item(Key={'id': f'token_{location_key}'})
            token_timestamp = token_response.get('Item', {}).get('timestamp') if 'Item' in token_response else None
        except Exception as e:
            logger.warning(f"Failed to get token timestamp: {str(e)}")
...
def validate_and_consume_token(location_key, provided_token):
    try:
        ...
        token_timestamp = old_item.get('timestamp')
        return True, "Token consumed successfully", token_timestamp
    except ClientError as e:
        if e.response['Error']['Code'] == 'ConditionalCheckFailedException':
            return False, "Invalid token or already used", None
        raise
