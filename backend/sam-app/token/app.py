...
def lambda_handler(event, context):
    ...
    try:
        ...
        # Handle token refresh for specific location
...
def refresh_location_token(location_key, event=None):
    ...
    # Broadcast token availability to all connections
    try:
...
def update_redis(timestamp, location_key, token, can_update):
    ...
    try:
        ...
        payload = ["SET", f"token_{location_key}", json.dumps(token_data)]
        ...
        if response.status_code == 200:
            ...
            logger.error(f"Failed to update Redis for {location_key} - Response: {response.text}")
            return False
