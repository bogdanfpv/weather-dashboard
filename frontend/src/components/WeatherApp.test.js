...
describe("WeatherApp", () => {
  ...
  test("displays current weather data correctly", async () => {
    ...
    const quickStatsRow =
      screen.getByTestId("high-temp").parentElement.parentElement;
  ...
  test("displays the current time on mount", async () => {
    ...
    await waitFor(() => {
      const now = fixedDate.toLocaleTimeString([], {
        hour: "2-digit",
        minute: "2-digit",
        hour12: true,
      });
  ...
  test("handles rate limiting correctly", async () => {
    ...
    const expectedTimeString = new Date(
      nextUpdateTime * 1000,
    ).toLocaleTimeString();
    ...
    expect(
      screen.getByText(new RegExp(`Available at ${expectedTimeString}`)),
    ).toBeInTheDocument();
  });
  ...
  test("handles fetch error", async () => {
    ...
    await waitFor(() => {
      expect(consoleSpy).toHaveBeenCalledWith(
        "Failed to fetch cached weather data:",
        expect.any(Error),
      );
    });
  ...
  test("displays notifications correctly", async () => {
    ...
    expect(
      screen.getByText("Weather data updated for Paris, FR"),
    ).toBeInTheDocument();
